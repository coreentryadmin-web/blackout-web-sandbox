...
- **Base URL:** `process.env.POLYGON_API_BASE ?? "https://api.massive.com"` (Polygon.io rebranded → Massive). Same key.
...
### HIGH-4 · `spx-power-hour-engine.ts` hardcodes `api.polygon.io` (ignores `POLYGON_API_BASE`) AND uses wrong query-param syntax
