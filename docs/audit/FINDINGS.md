...
- HIGH scoring: several factors not direction-mirrored — bullish news (+5..8) and institutional net-buying (+3) ADD to SHORT scores; congress signals ignore buy-vs-sell; dark-pool magnitude scores at 50% even when bias contradicts the thesis; gap-UP tag "gap-fill risk below" matches the SHORT branch's `"below"` keyword (+5).
