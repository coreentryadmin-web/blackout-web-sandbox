...
|---|---|---|---|---|---|
| **Polygon/Massive** (REST) | `providers/polygon.ts`, `polygon-options-gex.ts`, `polygon-largo.ts`, `gap-proxy.ts` | snapshots, chains, GEX, indices, news, search | `polygonTrackedFetch` → `acquirePolygonSlot` | token bucket 40rps + Redis global 40 + 5-consec-429 breaker; permissive/fail-open | `POLYGON_API_KEY`, `POLYGON_API_BASE` (default `api.massive.com`) |
