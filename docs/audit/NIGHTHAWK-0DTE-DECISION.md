...
| G-2 | **Opening-window block** | **user-directed 2026-07-13:** no NEW commits 9:30–9:45 ET only (first 15 minutes) — the original 10:30 spec here was overridden by the user, who wants open plays available; applies to BOTH engines (“0DTE” = Slayer + 0DTE Command; Slayer's BUY unlock moved 9:50→9:45). The 9:45–10:30 band stays open KNOWINGLY against F-4 and is measured by the calibration loop (`gate_calibration_json.committed_at_et`) so the boundary is revisited with per-play evidence | F-4 (overridden by user direction) | none — all of 7/13's entries were ≥9:50, so the four opening longs fall to **G-1**, not G-2 |
