...
- **Correctness $ leak (tier-independent):** `spx-power-hour-engine.ts` hardcodes `legacy-polygon-host` + wrong filter param syntax (`12-POLYGON HIGH-4`) → wastes a rate-limited call per build for a discarded synthetic-fallback result. Trivial $ but a clean correctness fix.
