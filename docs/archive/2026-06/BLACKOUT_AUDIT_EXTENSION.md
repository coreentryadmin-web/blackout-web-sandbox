...
**Top findings:**
- **HIGH-4:** `spx-power-hour-engine.ts:164-171` hardcodes `legacy-polygon-host` (ignores `polygon-base-env`) **and** uses underscore filter params (`strike_price_gte` vs the dotted `strike_price.gte` used everywhere else) → the engine **almost certainly always serves the synthetic fallback** (tier-independent correctness bug).
...
| 13 | **Fix `spx-power-hour-engine.ts`** — use `polygon-base-env` + dotted filter params; it almost certainly always serves the synthetic fallback | 12 HIGH-4 | all (correctness) |
