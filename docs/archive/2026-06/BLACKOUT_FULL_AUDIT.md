...
**Launch gating + adversarial sweeps + audit reconciliation (2026-06-24, later):**
- ✅ **Per-tool launch gate** (`7d89c35`): SPX Slayer + HELIX live to paid users; Largo / Heatmaps / Night Hawk locked behind a "Launching Soon" padlock (page + API + nav layers), admins bypass, unlock via the additive `LAUNCHED_TOOLS` env var. Admin path prod-verified.
- ✅ **Redis IPv6 fail-open cascade — live cause FIXED** (`888b7e3`): `make-redis` now connects with `family: 0`, so Railway's IPv6-only `*.railway.internal` is reachable (was `connect ETIMEDOUT` at boot → every limiter/cache/gate silently failing open). Clean-boot verified in prod.
- ✅ **Two adversarial multi-agent sweeps** (issue-sweep + launch-path): 26 code-verified findings → **25 fixed, 1 deferred** (server-cache cold-miss dedup, low/perf). Highlights: uw-socket teardown race, GEX-heatmap wall-`kind` false verdicts, spx-broadcaster `ErrorEvent` ReferenceError, closed-position immutability, polygon limiter breaker-block, token-bucket races, and the **billing/entitlement holes** — plan-ID lockout, `member:email:read` mass-downgrade (now fail-closed), per-user multi-email resolution, refund/chargeback revocation denylist, Night's Watch premium tier gate, `gex-positioning` launch-gate, and the `WHOP_WEBHOOK_SECRET` deploy-block on `/api/health`.
- ✅ **High-value open-code batch** (`eed7596`, `82f6d74`): gap-proxy limiter/breaker bypass, push/subscribe IDOR, org-wide AI-spend kill-switch lifted into `anthropic.ts`, GEX-heatmap cache LRU bound, `polygon-base-env` honored, FlowBrief hydration bug, verbose-error genericization, JSON-extraction `temperature: 0`, N+1 nighthawk insert → batch, cron-trigger timeout.

**📋 Definitive audit reconciliation (2026-06-24).** ~120 distinct findings across the 18 `docs/audit/*.md` files → **~40 RESOLVED** (verified in code), **~38 OPEN-CODE** (almost all non-blocking: DB write-path perf, frontend perf/CSS incl. I-01, AI-cost-per-call, provider edges), **~22 OPEN-OPERATOR** (PgBouncer + pool, replica pinning, the env vars `DAILY_AI_SPEND_KILL_USD` / `DISCORD_OPS_WEBHOOK_URL` / `DATABASE_SSL_STRICT` / `UW_MAX_RPS`, and the Redis fail-open policy), ~12 partial/accepted. **The launch-blocker cluster is done in code.** Items that need a PRODUCT/OPERATOR DECISION (not code): web-push + Sentry (install-or-descope), the I-01 CSS split, marketing-vs-product copy alignment (removed "Hunt Modes"; Heatmaps sold as sector/internals/tide but renders GEX only), a free-tier preview, and CSP `unsafe-inline` removal + flipping `eslint.ignoreDuringBuilds`.
