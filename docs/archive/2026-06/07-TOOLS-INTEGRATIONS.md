...
|---|---|---|---|---|
| **Polygon / Massive** (market data primary) | `POLYGON_API_KEY` / `MASSIVE_API_KEY`, base URL env (see `providers/polygon.ts`), `POLYGON_WS_INDICES` | `providers/polygon.ts`, `polygon-options-gex.ts`, `polygon-largo.ts`, `ws/polygon-socket.ts`, `ws/options-socket.ts` | Chains, GEX, indices, snapshots, Benzinga news/earnings, real-time index WS | Treated as "unlimited" (40 rps permissive limiter). **The base host is the Massive API host, NOT legacy polygon.io** — key must be a Massive key (per memory `project_gex_source.md`). |
