...
| **Q4** | Verdict-guard assert catches the FSM/counter desync it was built for | High | **Disagree — reopen with specific gap, do not close** | Idle-desync detection itself is genuinely fixed: the assert now explicitly checks `persistedState === "idle"` and throws (confirmed by direct trace + a passing test that exercises it without the `trigger_fired` field being stripped first — the prior no-op-test complaint is resolved). Test coverage and CI wiring (`PLAYBOOK_VERDICT_GUARD_ASSERT=1` in both `package.json`'s test script and `.github/workflows/ci.yml`) are also genuinely resolved. **But the core structural complaint from the fifth pass is unchanged**: the assert still consumes the *identical* `snapshots`/`armedPollCounts` objects that `applyPlaybookVerdictGuards` already consumed to produce the result being checked — traced end to end (`playbook-verdict-guard.ts:158-161`, and the production caller `playbook-match-resolver.ts:52-62`, a single DB load feeding both). `resolveEpisodeInstance`'s `from_state` is *provably identical* to `persisted.state` in every code path (both terminate at the same `snapshots[i].state` field) — so despite the code comment claiming to use "durable snapshot rows (not resolver-derived state)," there is no second, independent read anywhere. This means the assert can only ever catch an arithmetic bug inside `applyPlaybookVerdictGuards` itself — the exact real-world scenario it's supposed to guard against (an in-memory counter gone stale relative to a DB state that changed underneath it via a concurrent write or race) has no channel to be detected, because both paths see the same in-memory snapshot after the same single load. | **Re-open, narrowly.** The idle-check and test/CI gaps are closed — don't re-litigate those. What's still missing: either (a) have the assert perform an independent, fresh read of persisted instance state (a real second DB round-trip, accepting the latency cost, likely only worth doing behind the same opt-in flag) rather than reusing the caller's already-loaded `snapshots`, or (b) if a true independent read isn't worth the cost, rename/redocument this as a **self-consistency check** (which it genuinely and correctly is) rather than continuing to claim it catches a cross-source desync it structurally cannot see. Either resolution is small; the current state — claiming a guarantee the code doesn't provide — is the actual problem, not the assert's existence. |
...
| Q4 verdict assert | ✅ #100 (claimed) | **Re-open** — idle-desync detection + test/CI wiring are genuinely fixed, but the "independent source of truth" property is not real; assert reads the same in-memory snapshot the guard already consumed, so it cannot catch the actual cross-path/cross-tick desync scenario it's named for |
