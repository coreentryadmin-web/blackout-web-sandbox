...
Cron jobs are ECS scheduled tasks that hit `/api/cron/*` endpoints; they are not separate apps.
...
- **Primary prod app** (`blackout-web` on ECS) must set `allowedRedirectOrigins` including staging so
...
- Non-admin premium users only see tools where `isToolLaunched()` is true (SPX Slayer + HELIX by default;
  others need `LAUNCHED_TOOLS=heatmap,nighthawk,largo,grid` in the ECS task env).
...
- **Ops guardrails (no secret values):** `/admin` → Operations → **System Vitals** shows
  ...
  pooler hint from `DATABASE_URL` host. Arming `DAILY_AI_SPEND_KILL_USD` is via AWS Secrets
  Manager — the dashboard only reports posture.

### AWS ECS (production infrastructure)
- **Production:** ECS Fargate cluster `blackout-production-cluster` / service `blackout-production-web`.
  Healthcheck **`/api/ready`** (90s). Secrets via AWS Secrets Manager (`blackout-production/app/env`).
- **Staging:** ECS Fargate cluster `blackout-staging-cluster` / service `blackout-staging-web`
  at `https://staging.blackouttrades.com`. Secrets: `blackout-staging/app/env`.
- **Cron audit:** `npm run validate:cron` — checks `cron_job_runs` table for all 30 registered keys.
  GHA: `cron-audit-query.yml` (hourly RTH).
- **Postgres:** Production RDS with RDS Proxy. PITR via RDS automated backups.
  Restore drill: `docs/ops/PITR-RESTORE-DRILL.md`.
- **30 cron jobs** as ECS scheduled tasks, registered in `scripts/cron-keys.mjs`.
- **Still manual:** set `DISCORD_OPS_WEBHOOK_URL` / `DISCORD_PLAY_WEBHOOK_URL` in Secrets Manager.
...
- Opt-in: set `STOCKS_WS_ENABLED=1` (or `LULD_WS_ENABLED=1`) on ECS. Uses the same
...
- Staging RDS holds a **point-in-time Postgres copy** from ECS prod (see `blackout-infra/scripts/migrate-prod-postgres-to-staging-rds.mjs`). Re-sync weekly or before big tests.
...
| `npm run ops:collect:staging` | Staging ops action items (no ECS prod) |
...
4. After **09:35 ET**, confirm `spx-evaluate` cron + options-socket `authenticated` in ECS/CloudWatch logs.
