...
- **WebSockets are blocked** by the agent proxy (WS upgrades unsupported). UW/Polygon WS run **server-side** (on the AWS deploy — ECS Fargate); the browser gets data via **SSE + SWR polling**, so validate WS-sourced numbers through the REST/SSE endpoints that surface them.
...
- **Direct Postgres (raw TCP) is blocked**, same as WebSockets — a raw `/dev/tcp` connect to the AWS RDS Postgres endpoint host:port hangs/fails with no SYN-ACK, and the `pg` client never completes a TCP handshake (`timeout expired`). The private RDS endpoint is VPC-only by design; the public path is not reachable from this sandbox. Only HTTP(S) egress through the agent proxy works. So `pg_stat_activity`/lock/row-count probes against prod are **not possible from this sandbox** — root-causing a live DB-side issue (lock contention, slow query, table bloat) needs either an in-VPC shell/exec (e.g. ECS `execute-command` on a task in the AWS environment) or a temporary HTTP-exposed debug endpoint in the app itself. Don't spend time retrying a raw `pg.Client` connection here.
