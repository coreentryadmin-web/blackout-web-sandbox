...
function buildContext(detail: PositionDetail): string {
  ...
  if (s.positioning) {
    lines.push(
      `DEALER GAMMA: regime ${s.positioning.gammaRegime ?? "n/a"}, flip ${n(s.positioning.gammaFlip)}, max-pain ${n(s.positioning.maxPain)}, king ${n(s.positioning.kingStrike)}, walls ${s.positioning.walls.map((w) => `${w.kind} ${w.strike}`).join(", ") || "n/a"}.`
    );
  }
  if (s.flows) {
    lines.push(
