      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-12 border-b border-cyan-400/20 pb-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs font-mono uppercase tracking-widest text-slate-300 bg-white/5 border border-white/10 px-3 py-1 rounded-full">Post-Market</span>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="lg:w-64 flex-shrink-0">
            <div className="lg:sticky lg:top-8">
              ...
              <nav className="space-y-1">
                {sections.map((s) => (
                  <a key={s.id} href={`#${s.id}`} className="block text-sm text-slate-300 hover:text-cyan-400 hover:bg-cyan-400/5 transition-colors px-3 py-2 rounded-md border-l-2 border-transparent hover:border-cyan-400">
                    {s.label}
                  </a>
                ))}
              </nav>
              ...
              <div className="mt-6 p-4 bg-amber-400/5 border border-amber-400/20 rounded-xl">
                ...
                <p className="text-sm text-slate-300 leading-relaxed">Night Hawk plays are suggestions, not signals. Always validate at market open with live desk data.</p>
              </div>
            </div>
          </aside>
          ...
          <main className="flex-1 min-w-0 space-y-16">
            ...
            <section id="edition-structure">
              ...
              <p className="text-slate-300 leading-relaxed mb-6">Every Night Hawk edition follows a standardized six-block structure.</p>
                ...
                <div className="p-6 bg-cyan-400/5 border border-cyan-400/20 rounded-xl">
                  ...
                  <ul className="space-y-3">
                    {["Read the entire edition, including context and catalyst scan, before jumping to play ideas.", "Mark all three GEX levels on your chart before the open.", "Wait for the morning confirmation update before finalizing your plan.", "Validate the Night Hawk thesis with SPX Slayer, HELIX, and Heat Maps at open.", "Respect invalidation levels as hard stops on the thesis.", "Treat Night Hawk plays as directional frameworks and re-price strikes at open."].map((item, i) => (
                      <li key={i} className="flex gap-3 text-sm"><span className="text-cyan-400 flex-shrink-0 mt-0.5">+</span><span className="text-slate-300 leading-relaxed">{item}</span></li>
                    ))}
                  </ul>
                </div>
