...
/**
 * BATCH-load the durable rails for MANY sessions in ONE query (multi-day replay seed).
 * The per-session loader above costs one round-trip per session; a 15-session cold read
 * (Redis only hot-caches ~72h, so ~12 of 15 sessions always miss) would be 12+ sequential
 * round-trips on the SSR path. `session_ymd = ANY($2)` collapses that to one. Returns a map
 * keyed by session_ymd with each session's samples ascending by bucket; missing sessions are
 * simply absent. Never throws — {} on any guard miss or DB error, mirroring the single-session
 * loader's degrade-to-Redis contract.
 */
