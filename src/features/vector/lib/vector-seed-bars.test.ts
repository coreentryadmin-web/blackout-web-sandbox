...
test("fetchVectorSeedBars: seeds 15 sessions by default, sessionYmds ascending, latest start exposed", async () => {
  const queried: string[] = [];
  const res = await fetchVectorSeedBars(
    "SPX",
    new Date("2026-07-06T15:00:00Z"),
  ...
  assert.equal(res.sessionYmds.length, 15, "15 trading sessions included");
  assert.equal(res.sessionYmd, "2026-07-06");
  ...
  for (let i = 1; i < res.bars.length; i++) {
    assert.ok(res.bars[i]!.time > res.bars[i - 1]!.time, "strictly ascending across 15 sessions");
  }
});
...
test("fetchVectorSeedBars: newest 3 sessions stay 1m; older sessions are decimated to 5m", async () => {
  ...
  assert.equal(res.sessionYmds.length, 15);
  // 3 × 390 (1m) + 12 × 78 (5m) = 2106 bars.
  assert.equal(res.bars.length, 3 * 390 + 12 * 78);
...
test("fetchVectorSeedBars: bar-count ceiling drops whole OLDEST sessions, never the latest", async () => {
  // 3000 bars/session (pathological sub-minute-ish density): 3×3000 (1m) already busts the 6500
  ...
  assert.ok(res.bars.length <= 6500, `cap respected, got ${res.bars.length}`);
  assert.ok(res.sessionYmds.length >= 1 && res.sessionYmds.length < 15, "older sessions dropped");
});
