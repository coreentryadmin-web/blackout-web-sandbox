...
test("scoreWallIntegrity: a dominant, all-session wall scores FIRM", () => {
  const r = scoreWallIntegrity(walls.callWalls[0]!, "call", walls.callWalls, persistentHistory)!;
...
test("scoreWallIntegrity: a big but never-seen, clustered wall is penalized vs a persistent one", () => {
  // Same 55-pct put wall, but sitting next to a near-equal 50-pct wall (clustered)
  // and absent from the rail → should score below the firm call wall.
  const putTop = scoreWallIntegrity(walls.putWalls[0]!, "put", walls.putWalls, persistentHistory)!;
  const callTop = scoreWallIntegrity(walls.callWalls[0]!, "call", walls.callWalls, persistentHistory)!;
...
test("scoreWallIntegrity: no history → persistence is neutral 0.5 (never fabricated as proven)", () => {
  const r = scoreWallIntegrity(walls.callWalls[0]!, "call", walls.callWalls, [])!;
...
test("scoreWallIntegrity: single-wall side is fully isolated", () => {
  const one = scoreWallIntegrity({ strike: 7600, pct: 80 }, "call", [{ strike: 7600, pct: 80 }], []);
  assert.equal(one!.factors.isolation, 1);
});

test("scoreWallIntegrity: null/garbage wall → null, never throws", () => {
  assert.equal(scoreWallIntegrity({ strike: 0, pct: 50 }, "call", [], []), null);
  assert.equal(scoreWallIntegrity(undefined as never, "call", [], []), null);
});
